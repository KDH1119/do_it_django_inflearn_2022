...
class TestView(TestCase):
    ...
    def test_comment_form(self):
        ...
        # 로그인한 상태
        self.client.login(username='obama', password='ejrgus2458!')
        response = self.client.get(self.post_001.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
        comment_area = soup.find('div', id='comment-area')
    ...
    def test_comment_delete(self):
        ...
        # trump로 로그인 한 상태
        self.client.login(username='trump', password='ejrgus2458!')
        response = self.client.get(self.post_001.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
        ...
        # delete btn이 있어야함
        self.assertFalse(comment_area.find('a', id='comment-1-delete-btn'))#obama의 comment
        ...
        comment_area = soup.find('div', id='comment-area')
        self.assertNotIn('트롬프의 댓글입니다.', comment_area.text)
        self.assertEqual(Comment.objects.count(), 1)
    ...
    def test_update_post(self):
        ...
        tag_str_input = main_area.find('input', id='id_tags_str')
        self.assertIn('python; 파이썬 공부', tag_str_input.attrs['value'])
        ...
        self.assertIn('안녕안녕안녕', main_area.text)
